...
//PASSO 01: Ler quatidade de votos (validos, nulos e brancos) que cada candidato (A,B,C) recebeu.

	//CANDIDATO A	
	Double validoA = readWindowDouble("Digite os votos válidos do Candidato A");

	//CANDIDATO B
	Double validoB = readWindowDouble("Digite os votos válidos do Candidato B");

	//CANDIDATO C
	Double validoC = readWindowDouble("Digite os votos válidos do Candidato C");

	//VOTOS NULOS e BRANCOS
	Double nulos = readWindowDouble("Votos nulos");
	Double brancos = readWindowDouble("Votos brancos");

	
//PASSO 02: Calcular totais dos votos

	Double totalValidos = validoA + validoB + validoC;

	Double totalBrancoNulo = nulos + brancos;

	Double totalVotos = totalBrancoNulo + totalValidos; 
		

//PASSO 03: Calcular porcentagens

	Double percentValidosTotal = totalValidos * 100 / totalVotos;
	
	Double percentNulosTotal = nulos * 100 / totalVotos;
	
	Double percentBrancosTotal = brancos * 100 / totalVotos;
	
	
//CANDIDATO A
	Double percentValidoA = validoA * 100 / totalVotos;
	
	//CANDIDATO B
	Double percentValidoB = validoB * 100 / totalVotos;
	
	//CANDIDATO C
	Double percentValidoC = validoC * 100 / totalVotos;

	
//PASSO 04: EXIBIR RESULTADOS
	
		writeWindow(
		"         ||RESULTADOS DA ELEIÇÃO SINDICAL||\n" +
		"	\n" +
		"  Total de votos: " + totalVotos + "\n" +
		"	Votos válidos: " + totalValidos + "  |  " + String.format("%.2f", percentValidosTotal) + "%" + "\n" +
		"	  Votos Nulos:  " + nulos + "  |  " + String.format("%.2f",percentNulosTotal) + "%" + "\n" +
		"	Votos Brancos:  " + brancos + "  |  " + String.format("%.2f",percentBrancosTotal) + "%" + "\n" +
		"	\n" +
		"	\n" +
		"	Candidato A\n" +
		"	Votos válidos: " + validoA + "  |  " + String.format("%.2f",percentValidoA) + "%" + "\n" +
		"	\n" +
		"	Candidato B\n" +
		"	Votos válidos: " + validoB + "  |  " + String.format("%.2f",percentValidoB) + "%" + "\n" +
		"	\n" +
		"	Candidato C\n" +
		"	Votos válidos: " + validoC + "  |  " + String.format("%.2f",percentValidoC) + "%" + "\n"
		);
