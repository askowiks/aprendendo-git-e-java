...
// </editor-fold>
	...
	/*
Construir um programa que calcule e apresente em metros por segundo o valor da velocidade de um projétil
que percorre uma distância em quilômetros a um espaço de tempo em minutos. Utilize a fórmula
VELOCIDADE ← (DISTÂNCIA * 1000) / (TEMPO * 60).
		*/
	//PASSO 1: LER VARIAVEIS (DISTANCIA:KM) - (TEMPO:MIN)
		Double distancia = readWindowDouble("Distancia percorrida (em Km)");
		Double tempo = readWindowDouble("Tempo decorrido (em minutos)");
	
	//PASSO 2: CALCULAR VELOCIDADE => VELOCIDADE ← (DISTÂNCIA * 1000) / (TEMPO * 60)
		Double velocidade = (distancia * 100) / (tempo * 60);
	
	//PASSO 3: EXIBIR RESULTADO
		writeWindow(
				"O projétil percorre a distancia de " + distancia + " Km em " + tempo + " minutos \ncom velocidade média de " + velocidade + " m/s."
		);
