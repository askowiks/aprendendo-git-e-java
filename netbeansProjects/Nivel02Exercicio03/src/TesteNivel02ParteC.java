...
public class TesteNivel02ParteC {
...
/*
		...
		As pessoas serão deparadas em quartos de acordo com suas idades.
		00 - 03 ==> Campo do aprendiz
		04 - 10 ==> Campo dos escudeiros
		11 - 15 ==> Campo dos Pajens
		16 - 18 ==> Campo dos Cavaleiro
		19 - 30 ==> Campo dos arqueiro
		31 - 50 ==> Conselho do reino
		51 - 70 ==> Monte dos oráculos
		obs: Menores que 0 e maiores de 70 são proibidos.
		*/
		
		final String CABECALHO =
			"  | ACAMPAMENTO DO REINO |\n";
		...
		Integer idade = readWindowInteger(CABECALHO +
			" - Digite a sua idade -"
			);
		...
		if (idade < 0 || idade > 70) {
			auxMensagem = "Infelizmente esse acampamento não está preparado para receber você.";
		}
		else {
			auxMensagem += " Vá para o ";
		}
		
		if (idade >= 0 && idade <= 3){
			auxMensagem += "Campo do aprendiz";
		}
		
		if (idade >= 4 && idade <= 10){
			auxMensagem += "Campo dos escudeiros";
		}
		
		if (idade >= 11 && idade <= 15){
			auxMensagem += nome + "Campo dos Pajens";
		}
		if (idade >= 16 && idade <= 18){
			auxMensagem += "Campo dos Cavaleiros";
		}
		if (idade >= 19 && idade <= 30){
			auxMensagem += "Campo dos arqueiros";
		}
		if (idade >= 31 && idade <= 50){
			auxMensagem +=  "Conselho do reino";
		}
		
		if (idade >= 51 && idade <= 70){
			auxMensagem += "Monte dos oráculos";
		}
				
		
		
		writeWindow(CABECALHO + auxMensagem);
