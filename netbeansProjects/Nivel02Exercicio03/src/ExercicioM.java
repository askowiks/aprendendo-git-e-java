...
String nome = readWindowString("Digite seu nome");
String sexo = readWindowString("Digite seu sexo (M ou F)");

		if (sexo.equals("m")) {
			writeWindow("Ilmo. Sr. " + nome);
		
			if (sexo.equals("f")) {
			writeWindow("Ilma. Sra. " + nome);
			}
		}
			
			
		else {
			writeWindow("Sexo inválido");
		}
