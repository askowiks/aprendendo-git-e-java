...
// </editor-fold>
		...
		/*
Elaborar um programa que leia duas matrizes A e B de uma dimensão do tipo vetor
com dez elementos inteiros cada. Construir uma matriz C de mesmo tipo e dimensão
que seja formada pelo quadrado da soma dos elementos correspondentes nas matrizes
A e B. Apresentar os elementos da matriz C.
		...
		for (int indice = 0; indice < tamanhoMatriz; indice++) {
			matrizA[indice] = readInteger("[A] Digite um valor");
			
		}
		
		for (int indice = 0; indice < tamanhoMatriz; indice++) {
			matrizB[indice] = readInteger("[B] Digite um valor");
		...
		for (int indice = 0; indice < tamanhoMatriz; indice++) {
			...
			somaQuad = (int) (Math.pow(matrizA[indice], 2) + Math.pow(matrizB[indice], 2));
			
			matrizC[indice] = somaQuad;
			
		mensagem += matrizC[indice] + ", ";
		}
