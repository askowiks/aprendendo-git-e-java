...
/*
Elaborar um programa que leia uma matriz A de uma dimensão com seis elementos do
tipo real. Construir uma matriz B, em que cada posição de índice ímpar da matriz
B deve ser atribuída com um elemento de índice par existente na matriz A e cada
posição de índice par da matriz B deve ser atribuída com um elemento de índice
ímpar existente na matriz A. Apresentar os elementos das duas matrizes.
		...
		for (int indice = 0; indice < matrizA.length; indice++) {
			matrizA[indice] = readInteger("[A] Digite um número");
			msg += indice + ": " + matrizA[indice] + " | ";
		}
		msg += "\n \n";

		msg += "Matriz B:\n";
		for (int indice = 0; indice < matrizB.length; indice += 2) {
			matrizB[indice] = matrizA[indiceImpar];
			matrizB[indiceImpar] = matrizA[indice];
			indiceImpar += 2;
		}
