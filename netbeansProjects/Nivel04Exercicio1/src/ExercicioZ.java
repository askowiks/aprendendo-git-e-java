...
numéricos inteiros. Apresentar o total de elementos ímpares existentes na
matriz e também o percentual do valor total de números ímpares em relação à
quantidade total de elementos armazenados na matriz
		 */
		int tamanhoMatriz = 10;
		Integer[] matriz = new Integer[tamanhoMatriz];
		int valor = 0;
		int impares = 0;
		Double porcent = 0.0;
		String numImpar = "Números ímpares digitados: ";

		for (int indice = 0; indice < matriz.length; indice++) {
			valor = readInteger("Digite um valor");
			matriz[indice] = valor;
			if (valor % 2 != 0) {
				numImpar += "| " + valor + " |";
				impares++;
		...
		porcent = (impares * 100.0) / tamanhoMatriz;

		System.out.println(numImpar);
		System.out.println("Total de números ímpares: " + impares);
		System.out.println("Porcentagem de números ímpares: " + porcent + "%");
