...
primeira coluna deve ser formada pelos elementos da matriz A e a segunda coluna
pelos elementos da matriz B. Apresentar a matriz C.
		 */
		Integer[] matrizA = new Integer[7];
		Integer[] matrizB = new Integer[7];

		Integer[][] matrizC = new Integer[7][2];

		String mensagem = "";

		for (int indice = 0; indice < matrizA.length; indice++) {
			matrizA[indice] = readInteger("Digite o valor de A");
		}

		for (int indice = 0; indice < matrizA.length; indice++) {
			matrizB[indice] = readInteger("Digite o valor de B");
		}
		...
		for (int linha = 0; linha < matrizC.length; linha++) {
			...
			for (int coluna = 0; coluna < matrizC[linha].length; coluna++) {
				if (coluna == 0) {
					...
					matrizC[linha][coluna] = matrizB[linha];
				}
				mensagem += " |" + matrizC[linha][coluna];
			}
		}
